...
import { groupWorkoutsByWeeks, calculateWeekStats } from "@/lib/utils/workout-helpers";
...
describe("Workout Helpers", () => {
  ...
  describe("groupWorkoutsByWeeks", () => {
    ...
    it("should group 70 workout days into 10 weeks", () => {
      ...
      const workoutDays: WorkoutDayDTO[] = Array.from({ length: 70 }, (_, i) => createWorkoutDay(i + 1));
    ...
    it("should maintain workout day order within each week", () => {
      ...
      weeks.forEach((week) => {
        ...
        for (let i = 1; i < week.workoutDays.length; i++) {
          expect(week.workoutDays[i].day_number).toBeGreaterThan(week.workoutDays[i - 1].day_number);
        }
      });
    });
  });
